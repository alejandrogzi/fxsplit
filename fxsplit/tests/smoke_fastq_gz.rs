...
#[test]
fn fastq_gz_chunks_mean_records_per_file() {
    let t = tempdir();
    let input = join(t.path(), "input.fastq.gz");
    let outdir = join(t.path(), "out_chunks");

    write_gzip(&input, sample_fastq());
...
#[test]
fn fastq_gz_files_mean_number_of_output_files() {
    let t = tempdir();
    let input = join(t.path(), "input.fastq.gz");
...
#[test]
fn fastq_gz_rejects_headers_mode() {
    let t = tempdir();
    let input = join(t.path(), "input.fastq.gz");
